...
import com.chengning.redis.RedisService;
...
public class OrderController {
    ...
    public String deductStock(@PathVariable String productId) {
        ...
        try {
            ...
            boolean result =redisService.setIfAbsent(lockKey, "lock", 30);
            if (!result) {
            ...
            Integer stock = Integer.parseInt(redisService.get("stock"));
            if (stock > 0) {
                stock -= 1;
                redisService.set("stock", stock.toString());
    ...
    public boolean deductStockCAS(String lockKey, Integer count) {
        try {
            ...
            } while (!redisService.setIfAbsent(lockKey, "lock", 30));
            return true;
        } catch (Exception e) {
            return false;
        }
